           ...
           <table width="90%" border="0" cellspacing="0" cellpadding="0">
                    <tbody>
					  ...
					  ?>
                          ...
                          <tr>
                          <td width="52%" align="left" class="font_12">
                          ...
                          <?
						      ...
						      if ($this->kern->isMine($row['sender_adr'])==true)
								 ...
								 print "Multisignature protected address <strong>".$this->template->formatAdr($row['sender_adr'])."</strong> has sent you ".$row['amount']." ".$row['cur'].". The network is waiting for requested signatures. A minimum of ".$row['min']." signatures are required.";
							  
							   else  print "<strong>".$this->template->formatAdr($row['sender_adr'])."</strong> wants to send <strong>".$row['amount']." ".$row['cur']."</strong> to address <strong>".$this->template->formatAdr($row['rec_adr'])."</strong>. The sender is a multisig address and one of your addresses is an authorized signer."
						  ?>
                          ...
                          <?
						      if ($this->kern->isMine($row['signer_1'])==true && strlen($row['sign_1'])<10)
							     print "<a href='javascript:void(0)' onclick=\"$('#sign_modal').modal(); $('#trans_hash').val('".$row['trans_hash']."'); $('#signer').val('".$row['signer_1']."')\" class='btn btn-primary btn-sm'><span class='glyphicon glyphicon-ok'></span>&nbsp;&nbsp;Sign</a>";
								 
						      else if ($this->kern->isMine($row['signer_2'])==true && strlen($row['sign_2'])<10)
							     print "<a href='javascript:void(0)' onclick=\"$('#sign_modal').modal(); $('#trans_hash').val('".$row['trans_hash']."'); $('#signer').val('".$row['signer_2']."')\" class='btn btn-primary btn-sm'><span class='glyphicon glyphicon-ok'></span>&nbsp;&nbsp;Sign</a>";
								 
							  else if ($this->kern->isMine($row['signer_3'])==true && strlen($row['sign_3'])<10)
							     print "<a href='javascript:void(0)' onclick=\"$('#sign_modal').modal(); $('#trans_hash').val('".$row['trans_hash']."'); $('#signer').val('".$row['signer_3']."')\" class='btn btn-primary btn-sm'><span class='glyphicon glyphicon-ok'></span>&nbsp;&nbsp;Sign</a>";
								 
							  else if ($this->kern->isMine($row['signer_4'])==true && strlen($row['sign_4'])<10)
							     print "<a href='javascript:void(0)' onclick=\"$('#sign_modal').modal(); $('#trans_hash').val('".$row['trans_hash']."'); $('#signer').val('".$row['signer_4']."')\" class='btn btn-primary btn-sm'><span class='glyphicon glyphicon-ok'></span>&nbsp;&nbsp;Sign</a>";
								 
							  else if ($this->kern->isMine($row['signer_5'])==true && strlen($row['sign_5'])<10)
							     print "<a href='javascript:void(0)' onclick=\"$('#sign_modal').modal(); $('#trans_hash').val('".$row['trans_hash']."'); $('#signer').val('".$row['signer_5']."')\" class='btn btn-primary btn-sm'><span class='glyphicon glyphicon-ok'></span>&nbsp;&nbsp;Sign</a>";
						?>
